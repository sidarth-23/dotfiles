...
def check_git_command(command):
    ...
    if re.search(r"(^|\s)--no-verify(\s|$)", command):
        ...
        if not safe_in_message:
            print(
                "❌ Using --no-verify to skip Git Hooks validation is prohibited!",
                file=sys.stderr,
            )
            sys.exit(2)
    ...
    dangerous_patterns = [
        (
            r"git\s+push\s+.*\s+--force",
            "Force push may overwrite remote history, please confirm operation",
        ),
        (
            r"git\s+reset\s+--hard",
            "Hard reset will lose uncommitted changes, please confirm operation",
        ),
        (
            r"git\s+clean\s+-[fd]",
            "Clean operation will delete untracked files, please confirm operation",
        ),
    ]
