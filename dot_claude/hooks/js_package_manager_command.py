    ...
    if os.path.exists(pkg_json_path):
        try:
            with open(pkg_json_path, "r") as f:
                ...
                if (
                    any(k.startswith("eslint") for k in all_deps)
                    and "eslint" not in detected
                ):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                if config["fix_command"]:
                    results["fix_commands"].append(
                        f"{pm} exec -- {config['fix_command']}"
                    )
...
def format_output(results, pm):
    ...
    if not results["success"]:
        ...
        if results["fix_commands"]:
            ...
            output.append("")
            output.append(
                "For issues that can't be auto-fixed, manually edit the files."
            )
